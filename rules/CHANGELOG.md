...
- Add MCP OAuth 2.1 conformance rules and expand safe autofixes.
...
- b69b4d6: Fix four JS/TS rules that failed to parse on the shipped Opengrep engine and
...
- 97eaaae: Add an **MCP server-auth rule pack** (`mcp/` and `py/mcp/`) for the official Model Context Protocol SDKs — TypeScript (`@modelcontextprotocol/sdk`) and Python (`mcp` / FastMCP). Eight rules, mapped to CWE/OWASP, each with an enforced vulnerable/safe fixture:
...
- fa69cbe: Add NestJS and Fastify rule packs (7 low-false-positive rules for AI-generated Node backends).
...
- 4780eb2: Add 10 framework-aware, low-false-positive rules for the two frameworks AI
...
- 958db1a: Message polish. 26 rules now lead with a crisp one-sentence summary, so each
  rule's page title on oauthlint.dev and the CLI finding headline read as a clean
  line instead of running to four or five (the longest dropped from 265 to 130
  characters). Detection, metadata, and autofixes are unchanged; only the message
  prose was reworded.
...
- 9574a26: Precision fixes across the OAuth and flow rules, with no loss of real detection.
