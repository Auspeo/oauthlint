...
// The engine pins (version, per-platform assets, SHA-256 checksums) are the
// single source of truth in the `oauthlint` package, shared with the CLI.
import { ASSETS, CHECKSUMS, OPENGREP_VERSION, RELEASE_BASE } from 'oauthlint';

const execFileAsync = promisify(execFile);

// Re-exported so this module's surface (and its tests) are unchanged.
export { OPENGREP_VERSION, CHECKSUMS };
